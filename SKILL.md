...
description: 把视频 URL / PDF / 公认原理话题生成手绘图。默认出"双图组合"——视频内容总结 + 事实核查结论。用户要"原理 + 行动"图时追加第三张横版"原理推行动"图。视频走 NeuraRead API（YouTube/B站/抖音/小红书/播客/微信视频号），PDF 走 LlamaParse v2，统一用 GPT-5 + gpt-image-2 出图。触发：用户给链接/路径 + 说"生成手绘总结"、"画成手绘笔记"、"video sketchnote"、"复刻 ChatGPT 生图"、"原理图"、"该做什么不该做"，或直接 /video-sketchnote <url-or-pdf>。
---
...
- **本地 PDF 文件路径** → 调 LlamaParse v2 拿 markdown → GPT-5 提章节 → 出 **图 1：内容总结手账图** → 自动跑 claim-check → 出 **图 2：核查结论手账图**

**默认行为 = 两张图**。用户明确说"只要总结图 / 跳过核查 / 不用核查"才退化到单图。
...
### C. PDF → 全自动双图模式（LlamaParse + GPT-5 + gpt-image-2）
...
#### Step 2：核查 → 出审核结论图（**默认必做**）

复用 Step 1 已经从 LlamaParse 拿到的 markdown 当原文，对其中**可证伪的事实声明**（数字、引用研究、机构、产品、市场份额、临床结论等）做 claim-check 双向取证；纯方法论 / 综述类 PDF 没什么可核查的就报告"内容主要是综述/方法论，无强可核查事实点，跳过审核图"。
...
只在以下任意一种情况下跳过 Step 2（核查 + 审核图），其他都默认双图：
- 用户明确说："只要总结图 / 跳过核查 / 不用核查 / 别审核"
- 内容是纯个人经历 / 纯观点 / 纯方法论综述，没有可证伪事实点
- 总结图已耗时过长且 NeuraRead 数据明显不足（< 1000 字）

不要因为"内容看起来对"就跳过——半真型内容才最值得出审核图。
...
| 用户意图 | 默认产出 |
|---|---|
| 给链接说"生成手绘总结" | 总结图 + 审核图（2 张，竖版） |
| 链接 + "原理图 / 第一性原理 / 该做什么不该做" | 总结 + 审核 + 原理行动（3 张） |
| 链接 + "只要总结" | 仅总结图（1 张） |
| 链接 + "只要原理行动图" | 仅原理行动图（1 张，横版） |
